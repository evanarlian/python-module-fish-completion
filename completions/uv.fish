...
test -f $__fish_data_dir/completions/uv.fish; and source $__fish_data_dir/completions/uv.fish

complete -c uv -n "__fish_uv_using_subcommand run; and _python_module_uv_run_no_module" -s m -f -d 'Run library module as a script (terminates option list)' -xa '(uv run --no-sync --no-python-downloads python -c "import pkgutil; print(\'\n\'.join([p[1] for p in pkgutil.iter_modules()]))" 2>/dev/null)'
